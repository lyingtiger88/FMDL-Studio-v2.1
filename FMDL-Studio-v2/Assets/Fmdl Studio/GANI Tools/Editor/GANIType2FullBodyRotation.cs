...
/// </summary>
public class GANIType2FullBodyRotation : EditorWindow
{
    ...
    [MenuItem("FMDL Studio/GANI Tools/Type2 Full Body Rotation Builder")]
    public static void ShowWindow()
    {
        GANIType2FullBodyRotation window =
            GetWindow<GANIType2FullBodyRotation>("GANI Full Body Rotation");
    ...
    private void OnGUI()
    {
        ...
        GUILayout.Label(
            "MGSV Type2 / Mini-GANI - Full Body Rotation Builder",
            EditorStyles.boldLabel
        );

        EditorGUILayout.HelpBox(
            "Full Body Rotation combines the working torso/head/finger mapping with all four limb rotation groups that have now been observed to animate independently. " +
            "Root-diff and limb Vector3/IK tracks remain ignored by default, so this is a rotation-only full-body pass.",
            MessageType.Info
        );
        ...
        if (GUILayout.Button(
            "Build Full Body Rotation AnimationClip (.anim)",
            GUILayout.Height(42)))
        {
            BuildClip();
        }
