...
/// <summary>
/// MGSV Type2/Mini-GANI experimental Body Beta AnimationClip builder.
...
///
/// EXPERIMENTAL ROTATION-ONLY LIMB HYPOTHESIS:
...
/// U00 root-diff is NOT applied. Experimental limb groups default OFF so they can be isolated safely.
/// Source FMDL/GANI/TRK files are never modified.
/// </summary>
public class GANIType2BodyBeta : EditorWindow
{
    ...
    private bool includeLeftArm = false;
    private bool includeRightArm = false;
    private bool includeLeftLeg = false;
    private bool includeRightLeg = false;
    ...
    [MenuItem("FMDL Studio/GANI Tools/Type2 Body Beta Clip Builder")]
    public static void ShowWindow()
    {
        GANIType2BodyBeta window =
            GetWindow<GANIType2BodyBeta>("GANI Body Beta");
    ...
    private void OnGUI()
    {
        ...
        GUILayout.Label(
            "MGSV Type2 / Mini-GANI - Body Beta Clip Builder",
            EditorStyles.boldLabel
        );

        EditorGUILayout.HelpBox(
            "Body Beta keeps the working torso/head/finger mapping, then lets you test limb rotations one side at a time. " +
            "Experimental limb groups default OFF. Root-diff and limb Vector3 tracks remain ignored, so a bad hypothesis cannot move the whole character through space.",
            MessageType.Info
        );
        ...
        GUILayout.Label("Experimental limb isolation (rotation only)", EditorStyles.boldLabel);
        ...
        if (GUILayout.Button("All Experimental OFF"))
        {
        ...
        EditorGUILayout.HelpBox(
            "These mappings are hypotheses. For the first Beta test enable ONLY LEFT ARM. " +
            "The Vector3 tracks in U06/U08/U10/U12 are intentionally ignored because they look like IK/motion-point coordinates rather than local bone positions.",
            MessageType.Warning
        );
        ...
        if (GUILayout.Button(
            "Build Body Beta AnimationClip (.anim)",
            GUILayout.Height(42)))
        {
            BuildClip();
        }
        ...
        EditorGUILayout.BeginVertical("box");
        GUILayout.Label("Base mapping (kept from Alpha)", EditorStyles.boldLabel);
        ...
        GUILayout.Space(8);
        GUILayout.Label("Experimental rotation-only mapping", EditorStyles.boldLabel);
        ...
        GUILayout.Space(6);
        GUILayout.Label("Still ignored:", EditorStyles.boldLabel);
    ...
    private void BuildClip()
    {
        ...
        try
        {
            ...
            string assetPath =
                EditorUtility.SaveFilePanelInProject(
                    "Save partial GANI AnimationClip",
                    defaultName,
                    "anim",
            ...
            report.AppendLine("MGSV TYPE2 BODY BETA CLIP BUILD");
            report.AppendLine("============================================================");
            ...
            report.AppendLine("Experimental left arm : " + includeLeftArm);
            report.AppendLine("Experimental right arm: " + includeRightArm);
            report.AppendLine("Experimental left leg : " + includeLeftLeg);
            report.AppendLine("Experimental right leg: " + includeRightLeg);
            report.AppendLine();
            ...
            report.AppendLine();
            report.AppendLine("EXPERIMENTAL LIMB ROTATIONS");
            report.AppendLine("------------------------------------------------------------");

            if (includeLeftArm)
            {
                ApplyUnitRotation(clip, ganiData, layout, mini, 6, 0, bones["SKL_011_LUARM"], "EXP U06.S0 -> SKL_011_LUARM", report);
                ApplyUnitRotation(clip, ganiData, layout, mini, 7, 0, bones["SKL_012_LFARM"], "EXP U07.S0 -> SKL_012_LFARM", report);
                ApplyUnitRotation(clip, ganiData, layout, mini, 6, 2, bones["SKL_013_LHAND"], "EXP U06.S2 -> SKL_013_LHAND", report);
            }
            else
            {
            ...
            if (includeRightArm)
            {
                ApplyUnitRotation(clip, ganiData, layout, mini, 8, 0, bones["SKL_021_RUARM"], "EXP U08.S0 -> SKL_021_RUARM", report);
                ApplyUnitRotation(clip, ganiData, layout, mini, 9, 0, bones["SKL_022_RFARM"], "EXP U09.S0 -> SKL_022_RFARM", report);
                ApplyUnitRotation(clip, ganiData, layout, mini, 8, 2, bones["SKL_023_RHAND"], "EXP U08.S2 -> SKL_023_RHAND", report);
            }
            else
            {
            ...
            if (includeLeftLeg)
            {
                ApplyUnitRotation(clip, ganiData, layout, mini, 10, 1, bones["SKL_030_LTHIGH"], "EXP U10.S1 -> SKL_030_LTHIGH", report);
                ApplyUnitRotation(clip, ganiData, layout, mini, 11, 0, bones["SKL_031_LLEG"], "EXP U11.S0 -> SKL_031_LLEG", report);
                ApplyUnitRotation(clip, ganiData, layout, mini, 14, 0, bones["SKL_032_LFOOT"], "EXP U14.S0 -> SKL_032_LFOOT", report);
            }
            else
            {
            ...
            if (includeRightLeg)
            {
                ApplyUnitRotation(clip, ganiData, layout, mini, 12, 1, bones["SKL_040_RTHIGH"], "EXP U12.S1 -> SKL_040_RTHIGH", report);
                ApplyUnitRotation(clip, ganiData, layout, mini, 13, 0, bones["SKL_041_RLEG"], "EXP U13.S0 -> SKL_041_RLEG", report);
                ApplyUnitRotation(clip, ganiData, layout, mini, 15, 0, bones["SKL_042_RFOOT"], "EXP U15.S0 -> SKL_042_RFOOT", report);
            }
            else
            {
            ...
            UnityEngine.Debug.Log(
                "GANI partial AnimationClip created:\n" +
                assetPath +
                "\n\n" +
                report.ToString()
            );
            ...
            EditorUtility.DisplayDialog(
                "GANI Body Beta",
                "Partial AnimationClip created successfully.\n\n" +
                assetPath +
                "\n\n" +
                "Base mapping: WAIST/SPINE/CHEST/NECK/HEAD + both hands' finger tracks.\n" +
                "Experimental limb rotations were added only for the groups you enabled.\n" +
        ...
        catch (Exception ex)
        {
            UnityEngine.Debug.LogError(
                "GANI Body Beta build failed:\n" +
                ex
            );

            EditorUtility.DisplayDialog(
                "GANI Body Beta Error",
                ex.Message,
                "OK"
            );
        }
